    ...
    var wrapper = $( ".file_upload" ),
        ...
        $('.ratings-stars.ev .rating-star').hover(
            function(){
                ...
                $('.rating-star').removeClass('hov1 hov2 hov3 base-bg-color base-second-bg-color');
                if(index<2){
                     $('.rating-star').each(function(){if($(this).index()<=index) $(this).addClass('hov1 base-second-bg-color')});
                }
                if(index==2){
                     $('.rating-star').each(function(){if($(this).index()<=index) $(this).addClass('hov2')});
                }
                if(index>2){
                     $('.rating-star').each(function(){if($(this).index()<=index) $(this).addClass('hov3 base-bg-color')});
                }
            },
            function(){
                $('.rating-star').removeClass('hov1 hov2 hov3 base-bg-color base-second-bg-color');
                $('.rating-star.sel1').addClass('base-second-bg-color');
                $('.rating-star.sel3').addClass('base-bg-color');
            }
        );
        $('.ratings-stars.ev .rating-star').click(function(){
            var index=$(this).index();
            $('.rating-star').removeClass('sel1 sel2 sel3 hov1 hov2 hov3 base-bg-color base-second-bg-color');
            if(index<2){
                $('.rating-star').each(function(){if($(this).index()<=index) $(this).addClass('sel1 base-second-bg-color')});
            }
            if(index==2){
                $('.rating-star').each(function(){if($(this).index()<=index) $(this).addClass('sel2')});
            }
            if(index>2){
                $('.rating-star').each(function(){if($(this).index()<=index) $(this).addClass('sel3 base-bg-color')});
            }
            $('.ratings-stars .item input[type="hidden"]').val(index+1);
        });
